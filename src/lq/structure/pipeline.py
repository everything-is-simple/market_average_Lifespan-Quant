"""Structure 构建 pipeline — 批量生成结构位快照，支持分批构建与断点续传。

核心原则（七库全持久化纪律）：
    历史一旦发生就是永恒的瞬间——绝不重算。
    磁盘空间换内存，小批量断点续传。

用法：
    1. 全量构建：run_structure_build(signal_dates=[...], ...) — 首次初始化历史
    2. 日增量：run_structure_build(signal_dates=[today], ...) — 每日收盘后追加
    3. 断点续传：run_structure_build(..., resume=True) — 中断后从上次继续
"""

from __future__ import annotations

import json
import logging
from dataclasses import dataclass, field
from datetime import date, datetime
from pathlib import Path
from typing import Any, Sequence
from uuid import uuid4

import duckdb
import pandas as pd

from lq.core.paths import WorkspaceRoots, default_settings
from lq.core.resumable import prepare_resumable_checkpoint, save_resumable_checkpoint
from lq.structure.contracts import StructureSnapshot
from lq.structure.detector import build_structure_snapshot

logger = logging.getLogger(__name__)


# ---------------------------------------------------------------------------
# DuckDB schema
# ---------------------------------------------------------------------------

STRUCTURE_SCHEMA_SQL = """
-- 结构位快照表（主输出，标量字段 + JSON 细节）
CREATE TABLE IF NOT EXISTS structure_snapshot (
    code                       VARCHAR NOT NULL,
    signal_date                DATE    NOT NULL,
    has_clear_structure        BOOLEAN DEFAULT FALSE,
    nearest_support_price      DOUBLE,
    nearest_resistance_price   DOUBLE,
    available_space_pct        DOUBLE,
    support_count              INTEGER DEFAULT 0,
    resistance_count           INTEGER DEFAULT 0,
    recent_breakout_type       VARCHAR,
    recent_breakout_date       DATE,
    recent_breakout_confirmed  BOOLEAN,
    detail_json                VARCHAR,
    run_id                     VARCHAR,
    created_at                 TIMESTAMP DEFAULT current_timestamp,
    PRIMARY KEY (code, signal_date)
);

-- 构建 manifest
CREATE TABLE IF NOT EXISTS structure_build_manifest (
    run_id         VARCHAR PRIMARY KEY,
    status         VARCHAR NOT NULL,
    asof_date      DATE,
    stock_count    INTEGER DEFAULT 0,
    created_at     TIMESTAMP DEFAULT current_timestamp
);
"""

# 写入数据库的列名顺序
_SNAPSHOT_COLS = (
    "code", "signal_date", "has_clear_structure",
    "nearest_support_price", "nearest_resistance_price",
    "available_space_pct", "support_count", "resistance_count",
    "recent_breakout_type", "recent_breakout_date",
    "recent_breakout_confirmed", "detail_json",
    "run_id", "created_at",
)


# ---------------------------------------------------------------------------
# 初始化
# ---------------------------------------------------------------------------

def bootstrap_structure_storage(structure_db_path: Path) -> None:
    """初始化 structure 数据库 schema（幂等）。"""
    structure_db_path.parent.mkdir(parents=True, exist_ok=True)
    with duckdb.connect(str(structure_db_path)) as conn:
        conn.execute(STRUCTURE_SCHEMA_SQL)


# ---------------------------------------------------------------------------
# 批量构建结果
# ---------------------------------------------------------------------------

@dataclass
class StructureBuildResult:
    """Structure 构建结果摘要。"""

    run_id: str = field(
        default_factory=lambda: f"struct-{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}-{uuid4().hex[:8]}"
    )
    dates_total: int = 0
    dates_completed: int = 0
    dates_skipped: int = 0
    rows_written: int = 0
    errors: int = 0
    status: str = "completed"


# ---------------------------------------------------------------------------
# 辅助查询
# ---------------------------------------------------------------------------

def list_stock_codes(market_base_path: Path) -> list[str]:
    """从 market_base 获取所有有日线数据的股票代码。"""
    with duckdb.connect(str(market_base_path), read_only=True) as conn:
        rows = conn.execute(
            "SELECT DISTINCT code FROM stock_daily_adjusted ORDER BY code"
        ).fetchall()
    return [r[0] for r in rows]


def list_trading_dates(
    market_base_path: Path,
    start: date,
    end: date,
) -> list[date]:
    """从 market_base 获取指定范围内有日线数据的交易日。"""
    with duckdb.connect(str(market_base_path), read_only=True) as conn:
        rows = conn.execute(
            "SELECT DISTINCT trade_date FROM stock_daily_adjusted "
            "WHERE trade_date >= ? AND trade_date <= ? "
            "ORDER BY trade_date",
            [start, end],
        ).fetchall()
    return [r[0] for r in rows]


# ---------------------------------------------------------------------------
# 核心构建函数
# ---------------------------------------------------------------------------

def run_structure_build(
    *,
    market_base_path: Path,
    structure_db_path: Path,
    signal_dates: Sequence[date],
    codes: Sequence[str] | None = None,
    batch_size: int = 200,
    resume: bool = False,
    reset_checkpoint: bool = False,
    settings: WorkspaceRoots | None = None,
    verbose: bool = True,
) -> StructureBuildResult:
    """Structure 全量/增量构建，支持断点续传。

    核心流程：
        1. 按日期逐日处理
        2. 每日内按 batch_size 分批处理股票
        3. 每批完成立即写入 structure.duckdb
        4. 每个日期完成后保存 checkpoint
        5. resume=True 时从最后完成的日期继续

    参数：
        market_base_path    — market_base.duckdb（只读）
        structure_db_path   — structure.duckdb（读写）
        signal_dates        — 待构建日期列表
        codes               — 股票代码列表（None = 全市场）
        batch_size          — 每批股票数（控制内存，默认 200）
        resume              — 从 checkpoint 续跑
        reset_checkpoint    — 清空旧 checkpoint 重跑
        settings            — WorkspaceRoots（用于 checkpoint 路径）
        verbose             — 打印进度
    """
    if not signal_dates:
        return StructureBuildResult(status="empty")

    bootstrap_structure_storage(structure_db_path)

    # 解析股票代码
    if codes is None:
        codes = list_stock_codes(market_base_path)
        if verbose:
            print(f"自动获取全市场股票：{len(codes)} 只")
    all_codes = list(codes)

    result = StructureBuildResult(dates_total=len(signal_dates))

    # 准备 checkpoint
    if settings is None:
        settings = default_settings()
    fingerprint = {
        "structure_db": str(structure_db_path),
        "dates_range": f"{signal_dates[0]}..{signal_dates[-1]}",
        "codes_count": len(all_codes),
    }
    store, state = prepare_resumable_checkpoint(
        checkpoint_path=None,
        settings_root=settings,
        domain="structure",
        runner_name="build_snapshot",
        fingerprint=fingerprint,
        resume=resume,
        reset_checkpoint=reset_checkpoint,
    )

    # 恢复已完成日期
    completed_dates: set[str] = set()
    if state is not None:
        completed_dates = set(state.get("completed_dates", []))
        if verbose:
            print(f"从 checkpoint 恢复：已完成 {len(completed_dates)} 个日期")

    # 标记运行中
    save_resumable_checkpoint(store, fingerprint=fingerprint, payload={
        "status": "running",
        "completed_dates": sorted(completed_dates),
        "run_id": result.run_id,
    })

    # 逐日处理
    with duckdb.connect(str(market_base_path), read_only=True) as base_conn:
        for idx, sig_date in enumerate(signal_dates):
            date_key = sig_date.isoformat()

            if date_key in completed_dates:
                result.dates_skipped += 1
                continue

            if verbose:
                print(
                    f"  [{idx + 1}/{len(signal_dates)}] {date_key}",
                    end="", flush=True,
                )

            date_rows = 0
            date_errors = 0

            for b_start in range(0, len(all_codes), batch_size):
                batch_codes = all_codes[b_start:b_start + batch_size]
                rows = _compute_batch(base_conn, batch_codes, sig_date)
                if rows:
                    _flush_batch(structure_db_path, rows, result.run_id)
                    date_rows += len(rows)
                date_errors += len(batch_codes) - len(rows)

            result.rows_written += date_rows
            result.errors += date_errors
            result.dates_completed += 1
            completed_dates.add(date_key)

            if verbose:
                msg = f" → {date_rows} 行"
                if date_errors:
                    msg += f"（失败 {date_errors}）"
                print(msg)

            save_resumable_checkpoint(store, fingerprint=fingerprint, payload={
                "status": "running",
                "completed_dates": sorted(completed_dates),
                "run_id": result.run_id,
                "last_date": date_key,
            })

    # 完成状态
    if result.errors > 0 and result.dates_completed == 0:
        result.status = "failed"
    elif result.errors > 0:
        result.status = "partial"

    save_resumable_checkpoint(store, fingerprint=fingerprint, payload={
        "status": "done",
        "completed_dates": sorted(completed_dates),
        "run_id": result.run_id,
    })
    _write_manifest(structure_db_path, result, signal_dates[-1])

    if verbose:
        print(
            f"\n完成：{result.dates_completed} 日完成 / "
            f"{result.dates_skipped} 日跳过 / {result.dates_total} 日总计"
        )
        print(f"写入 {result.rows_written} 行，失败 {result.errors}")

    return result


# ---------------------------------------------------------------------------
# 内部辅助
# ---------------------------------------------------------------------------

def _compute_batch(
    base_conn: duckdb.DuckDBPyConnection,
    codes: list[str],
    signal_date: date,
) -> list[dict[str, Any]]:
    """为一批股票在指定日期计算结构位快照。"""
    results: list[dict[str, Any]] = []
    for code in codes:
        try:
            # 读取日线数据（detector 需要 date/adj_high/adj_low/adj_close 列）
            daily_df = base_conn.execute(
                "SELECT trade_date AS date, "
                "       open AS adj_open, high AS adj_high, "
                "       low AS adj_low, close AS adj_close, "
                "       volume AS adj_volume "
                "FROM stock_daily_adjusted "
                "WHERE code = ? AND adjust_method = 'backward' "
                "  AND trade_date <= ? "
                "ORDER BY trade_date",
                [code, signal_date],
            ).df()

            if daily_df.empty:
                continue

            snap = build_structure_snapshot(code, signal_date, daily_df)
            results.append(_snapshot_to_row(snap))
        except Exception as exc:
            logger.debug("Structure 计算失败 %s@%s: %s", code, signal_date, exc)
    return results


def _snapshot_to_row(snap: StructureSnapshot) -> dict[str, Any]:
    """StructureSnapshot → 数据库行（标量字段 + JSON 细节）。"""
    return {
        "code": snap.code,
        "signal_date": snap.signal_date,
        "has_clear_structure": snap.has_clear_structure,
        "nearest_support_price": (
            snap.nearest_support.price if snap.nearest_support else None
        ),
        "nearest_resistance_price": (
            snap.nearest_resistance.price if snap.nearest_resistance else None
        ),
        "available_space_pct": snap.available_space_pct,
        "support_count": len(snap.support_levels),
        "resistance_count": len(snap.resistance_levels),
        "recent_breakout_type": (
            snap.recent_breakout.breakout_type if snap.recent_breakout else None
        ),
        "recent_breakout_date": (
            snap.recent_breakout.event_date if snap.recent_breakout else None
        ),
        "recent_breakout_confirmed": (
            snap.recent_breakout.confirmed if snap.recent_breakout else None
        ),
        "detail_json": json.dumps(
            snap.as_dict(), ensure_ascii=False, default=str,
        ),
    }


def _flush_batch(
    structure_db_path: Path,
    rows: list[dict],
    run_id: str,
) -> None:
    """写入一批结构位快照（先删后插，幂等）。"""
    _batch_df = pd.DataFrame(rows)
    _batch_df["run_id"] = run_id
    _batch_df["created_at"] = datetime.utcnow()

    sig_date = _batch_df["signal_date"].iloc[0]
    batch_codes = _batch_df["code"].unique().tolist()

    col_list = ", ".join(_SNAPSHOT_COLS)
    with duckdb.connect(str(structure_db_path)) as conn:
        conn.execute(
            "DELETE FROM structure_snapshot "
            "WHERE signal_date = ? AND code = ANY(?)",
            [sig_date, batch_codes],
        )
        conn.execute(
            f"INSERT INTO structure_snapshot ({col_list}) "
            f"SELECT {col_list} FROM _batch_df"
        )


def _write_manifest(
    structure_db_path: Path,
    result: StructureBuildResult,
    asof_date: date,
) -> None:
    """写入构建 manifest 记录。"""
    with duckdb.connect(str(structure_db_path)) as conn:
        conn.execute(
            "INSERT OR REPLACE INTO structure_build_manifest "
            "VALUES (?, ?, ?, ?, current_timestamp)",
            [result.run_id, result.status, asof_date, result.rows_written],
        )
