...
| 七数据库架构 | raw_market / market_base / malf / structure / filter / research_lab / trade_runtime |
| MALF 主问题意识 | 从父系统继承“市场平均寿命”问题框架，但本系统已把正式执行主轴重定向为 `long_background_2 / intermediate_role_2 / malf_context_4` + 生命周期三轴排位 |
| PAS 触发器治理 | 继承父系统验证体系，但当前正式地位已收敛为：BOF=MAINLINE，PB/TST=CONDITIONAL，BPB/CPB=REJECTED |
...
| **filter 模块** | A4 | 不利市场条件过滤器：5 类条件检测；结果按日按股持久化 |
...
| `malf` | 四格上下文 + 生命周期三轴排位计算，输出 `execution_context_snapshot` 桥表 | malf |
| `structure` | 统一结构位合同：波段高低点、支撑阻力位、突破分类；结果按日按股持久化 | structure |
| `filter` | 不利市场条件过滤器：5 类条件检测；结果按日按股持久化 | filter |
| `alpha/pas` | 五触发器治理与正式信号落库；当前主线只允许 BOF，PB/TST 为条件准入，BPB/CPB 为 rejected | research_lab |
...
| L3 | malf | `data/malf/` | malf | 四格上下文 + 生命周期三轴排位快照 | 新月/新周 |
...
**`config_hash` 增量跳过机制**：每行携带输入参数的哈希。同一 `(code, date, config_hash)` 组合已存在 → 直接跳过，不重算。参数变更 → selective rebuild 受影响行。
...
| 触发器 | 状态 | 说明 |
|---|---|---|
| BOF | **MAINLINE** | 主线 trigger，允许在 system 主线中调用 |
| TST | CONDITIONAL | 条件 trigger，需在限定背景与准入口径下使用 |
| PB | CONDITIONAL | 条件 trigger，当前主要保留第一 PB 等受控研究路径 |
| BPB | **REJECTED** | 永久拒绝，system 层禁止调用 |
| CPB | **REJECTED** | 当前主线已拒绝，system 层禁止调用 |
...
| 执行纪律 | 有闭环传统但未显式目录化 | execution 目录显式要求 card / evidence / record / conclusion 四件套 |

### 8.3 与本系统 v0.0 草案的差异

开源选型决策（2026-04-04 终裁）
...
| **tushare** | 仅审计 | 个人项目，HTTP API，仅用于复权因子交叉校验 |
| **baostock** | 仅审计 | 个人项目，仅用于第二校准源 |
...
5. **不用其回测引擎**：本系统 BacktestEngine 自实现，保持系统架构自主权；设计理念可借鉴，包依赖不引入
...
| malf | `modules/malf/00-malf-charter-20260331.md` | 四格上下文 + 生命周期三轴排位 + execution_context_snapshot 设计 |
