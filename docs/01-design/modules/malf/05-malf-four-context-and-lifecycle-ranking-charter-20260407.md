...
下列概念允许保留追溯价值，但不再代表本章程的正式主读数：

| 概念 | 当前地位 | 放弃原因 |
|------|----------|----------|
| `monthly_state_8` | 月线诊断 / 兼容字段 | 八态会先切碎样本，不能代替四格上下文与三轴寿命统计 |
| `weekly_flow_relation_to_monthly` | 历史命名 / 兼容别名 | 正式执行口径统一为 `intermediate_role_2` |
| `16-cell` / “月线 × 周线 × PAS”矩阵 | 历史组织方式 | 这类矩阵可以描述背景与触发器分布，但无法直接表达中级波段生命位置 |
| `scene quartile` / 裸 `quartile` | 观察字段 / 旧报表字段 | 四分位只是压缩表达，且 `scene quartile` 不是生命周期三轴原始排位 |

因此，今后任何 runner、桥表、PAS 接口、position 接口若仍出现上述概念，都必须显式标注其为兼容残留，不得冒充当前章程主轴。
