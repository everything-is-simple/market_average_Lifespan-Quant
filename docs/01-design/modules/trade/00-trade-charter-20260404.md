...
```
research_lab.pas_formal_signal
    ↓ trade 内部构造 PasSignal
compute_position_plan()
    ↓
TradeManager.update()
    ↓
trade_record / trade_build_manifest
```

**核心职责**：在当前最小实现中，`trade` 直接消费 `research_lab.pas_formal_signal` 与 `market_base` 日线，并在模块内部桥接 `position.sizing` 完成回测；`PositionPlan / PositionExitPlan + Broker + BacktestEngine + 成本模型` 仍保留为目标态扩展方向。

---

## 3. 正式输入

1. `research_lab.pas_formal_signal`（当前主线正式输入）
2. `market_base.stock_daily_adjusted`（T+1 开盘价、每日 OHLCV）
3. `position.compute_position_plan()`（当前在 trade 内部调用的桥接入口）
4. `PositionExitPlan` / `raw_market.raw_xdxr_event` 仍属于目标态设计入口，当前 `run_trade_build()` 尚未核实为主线必需输入
...
| `TradeRunSummary` | `dataclass` | 合同已定义；当前 schema 已有 `trade_run_summary`，但 `run_trade_build()` 尚未核实实际写入 |
| 权益曲线 | DuckDB 表 | 目标态输出，当前最小实现未落地 |
| `BrokerOrderInstruction` | `dataclass` | 实盘适配目标态，当前最小实现未落地 |
...
2. `trade/pipeline.py` 最小回测路径：读 PAS 信号 → 内部桥接 sizing → TradeManager 模拟 → 写 `trade_record`
3. `trade_runtime.duckdb` 当前最小写入子集（`trade_record` + `trade_build_manifest`；`trade_run_summary` schema 已定义但未核实写入）
4. 冻结 Broker / BacktestEngine / 成本模型 / BrokerAdapter 的目标态边界，但不把它们冒充为当前已落地主线
...
6. **最小主线不得伪装成完整版 Broker 路径**：当前已核实主线是 `pas_formal_signal → compute_position_plan() → TradeManager`，在 Broker / PositionExitPlan / BacktestEngine 未落地前，不得把目标态写成现状
7. **成本模型属于目标态正式边界**：在 commission / stamp_duty / transfer_fee / slippage 未实现前，文档不得宣称当前 trade 回测已完整扣除成本
...
10. **trade_runtime 为持久化库**：交易记录一旦产生就是历史事实，按交易增量追加；`config_hash` 仍是全局治理方向，不得在本章程中假定当前最小 schema 已完整落地
...
2. `run_trade_build()` 可逐日读取 `pas_formal_signal`，并通过 `compute_position_plan() + TradeManager` 产出 `TradeRecord`
3. `TradeRecord` 可按 `trade_id / code / signal_date` 追溯
4. `trade_runtime.duckdb` 当前最小 schema 已初始化；`trade_record` 与 `trade_build_manifest` 已有实际写入路径
5. `trade_run_summary` 是否落库必须以真实写入逻辑为准，不得仅因 schema 存在就视为完成
6. Broker / BacktestEngine / 成本模型 / BrokerAdapter 的落地必须另开卡，不属于当前最小完成标准
