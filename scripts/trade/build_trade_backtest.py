"""交易回测构建脚本 — 从 PAS 信号到交易记录，分批构建 + 断点续传。
...
用法：
    ...
    # 指定股票
    python scripts/trade/build_trade_backtest.py --date 2026-04-07 --codes 600000.SH 600006.SH

    # 参数预览
    python scripts/trade/build_trade_backtest.py --date 2026-04-07 --dry-run
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="交易回测构建：research_lab + market_base → trade_runtime.duckdb"
    )
    ...
    p.add_argument(
        ...
        help="只处理指定股票代码，不指定则不过滤",
    )
    p.add_argument(
        "--strategy", default="pas_default",
        help="策略名称标识，默认 pas_default",
    )
    p.add_argument(
...
def main() -> None:
    ...
    from lq.trade.pipeline import run_trade_build
    ...
    print(f"  trade_runtime:  {dbs.trade_runtime}")
    ...
    result = run_trade_build(
        ...
        trade_db_path=dbs.trade_runtime,
